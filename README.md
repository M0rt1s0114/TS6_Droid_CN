...
### 安全
- 服务器密码与 TS 身份私钥改用 **Android Keystore AES-GCM** 加密存储，旧版本明文数据启动时自动迁移并清除
- 加密数据损坏时自动回退，不因凭据问题导致崩溃

### 稳定性修复
- 连接状态从每 0.5 秒轮询改为服务事件推送，降低后台与待机耗电
- 连接失败现在显示具体原因（密码错误 / 服务器已满 / 被封禁 / 超时 / 地址无效等）
- 修复 `2.1.3-2` 发布版启动即崩溃（连接状态在轮询协程前初始化）
- Android 14+ `FOREGROUND_SERVICE_TYPE_SPECIAL_USE` 缺少必需声明导致的启动异常
- 悬浮窗权限检查不再在未启用悬浮窗时阻断连接
- 语言/主题设置读取失败时不再导致启动崩溃
- 前台服务增加唤醒锁与采集看门狗，锁屏后麦克风保持工作
- 服务协程增加全局异常兜底，未捕获异常不再直接崩进程
- 崩溃与关键事件日志落盘（`Android/data/com.yuaxi.ts6droid.cn/files/logs/`）
- 悬浮窗“退出”现在真正断开服务器，且时长记录失败也不会跳过断开

### 音频与交互
- 通知栏常驻通知增加三个快捷按钮：**麦克风静音 / 扬声器静音 / 断开连接**
- 集成 **sherpa-onnx DPDFNet 48kHz AI 降噪**（开关：设置 → 音频 → 麦克风降噪；失败自动回退系统降噪）
- 扬声器静音强制联动麦克风静音，防止“关了声音却忘了关麦”
- PTT 按住说话松手自动恢复静音；麦克风开关状态与真实静音状态同步
- 语音队列满时丢最旧帧，解码缓冲残留清零，减少杂音

### UI 与体验
- 悬浮窗**创建时即自动贴边**，无需手动拖拽；展开/收起状态下拖拽松手后也会自动贴边，并记住上次停靠位置
- 新增浅色 / 深色 / **AMOLED 纯黑**主题模式，并支持动态壁纸文字可读性遮罩
- 统一“悬浮磁贴”设计语言：首页服务器卡片、服务器页标题/频道区、底部工具栏
- 首页服务器卡片显示：当前昵称、在线人数、累计连接时长
- 服务器页返回不再断开连接；卡片提供“再次进入 / 断开连接”
- 进入服务器后标题栏显示实时在线人数与本次会话计时
- 移除在线二次元壁纸模式，保留自定义背景上传（减小体积、避免随机图破坏可读性）
- 设置页参考 HyperOS 结构重做；私信聊天参考 Telegram 风格
- 工具链升级：Compose BOM 2026.05、Kotlin 2.4、AGP 8.13、Coil 3、compileSdk/targetSdk 36
- Lint 全项目 **0 error**
